// CanvasObject.cpp
// Implementation of the base class synchronization logic for position, size, and rotation.

#include "CanvasObject.h"
#include "VectorUtils.h" // convenience operators for sf::Vector2f math
...
void CanvasObject::setPosition(const sf::Vector2f& pos) {
    m_position = pos; x_ = pos.x; y_ = pos.y;
}
...
void CanvasObject::setSize(const sf::Vector2f& size) {
    m_size = size; width_ = size.x; height_ = size.y;
}
...
void CanvasObject::setRotation(float degrees) {
    ...
    m_rotationDegrees_sfml = degrees; // mirror for SFML draw state
}
